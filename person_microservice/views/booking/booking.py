...
from schemas import workdays_schema, appointments_schema, workday_schema, appointment_schema
...
from services.booking.workday import send_message_about_cancelled_appointment, delete_all_appointment_in_workday, validation_access
...
def get_workday_by_user_id(user_id):
    ...
    return Response(workdays_schema.dumps(workdays), status=200, mimetype="application/json")
...
def create_workday(user_id):
    ...
    return Response(workday_schema.dumps(new_workday), status=201, mimetype="application/json")


def create_appointments_for_workday(workday_data, workday):
    for start_time in get_start_time_appointments(workday_data["start_time"], workday_data["end_time"], workday_data["appointment_duration"]):
...
def get_workday_by_id(user_id, workday_id):
    ...
    return Response(workday_schema.dumps(workday), status=200, mimetype="application/json")
...
@booking_view.route('/user/<user_id>/doctor/workday/<workday_id>/appointments', methods=["Get"])
def get_appointments(user_id, workday_id):
    ...
    return Response(appointments_schema.dumps(appointments), status=200, mimetype="application/json")


@booking_view.route('/user/<user_id>/doctor/workday/<workday_id>/appointments/<appointment_id>', methods=["Get"])
def get_appoinment_by_id(user_id, workday_id, appointment_id):
    ...
    return Response(appointment_schema.dumps(appointment), status=200, mimetype="application/json")


@booking_view.route('/user/<user_id>/doctor/workday/<workday_id>/appointments/<appointment_id>', methods=["Put"])
def update_appointment(user_id, workday_id, appointment_id):
    ...
    Appointment.query.filter((Appointment.id == appointment_id)).update(appointment_data)
    db.session.commit()
    
    return Response(status=200)


@booking_view.route('/user/<user_id>/doctor/workday/<workday_id>/appointments/<appointment_id>', methods=["Delete"])
